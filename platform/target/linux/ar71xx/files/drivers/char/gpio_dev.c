...
gpio_mod_init(void)
{
	...
	result = register_chrdev(GPIO_DEV_MAJOR, DEVNAME, &gpio_fops);
	if (result < 0)
	{
		printk(KERN_ERR DRVNAME ": Error gpio mod init!\n");
		return result;
...
gpio_mod_exit(void)
{
	unregister_chrdev (GPIO_DEV_MAJOR, DEVNAME);
}
